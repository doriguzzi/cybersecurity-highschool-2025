...
def parse_packet(pkt):
    ...
    try:
        ...
        pf.features_list.append(0 if (pkt.ip.flags_df==False) or (pkt.ip.flags_df==0) else 1) # don't fragment
        pf.features_list.append(0 if (pkt.ip.flags_mf==False) or (pkt.ip.flags_mf==0) else 1) # more fragments
        pf.features_list.append(0 if (pkt.ip.flags_rb==False) or (pkt.ip.flags_rb==0) else 1) # reserved bit
        ...
        if pkt.transport_layer != None:
            if protocol == socket.IPPROTO_TCP:
                ...
                pf.features_list.append(0 if (pkt.tcp.flags_ack==False) or (pkt.tcp.flags_ack==0) else 1) # Acknowledgment
                pf.features_list.append(0 if (pkt.tcp.flags_cwr==False) or (pkt.tcp.flags_cwr==0) else 1)
                pf.features_list.append(0 if (pkt.tcp.flags_ece==False) or (pkt.tcp.flags_ece==0) else 1)
                pf.features_list.append(0 if (pkt.tcp.flags_fin==False) or (pkt.tcp.flags_fin==0) else 1)
                pf.features_list.append(0 if (pkt.tcp.flags_push==False) or (pkt.tcp.flags_push==0) else 1)
                pf.features_list.append(0 if (pkt.tcp.flags_res==False) or (pkt.tcp.flags_res==0) else 1)
                pf.features_list.append(0 if (pkt.tcp.flags_reset==False) or (pkt.tcp.flags_reset==0) else 1)
                pf.features_list.append(0 if (pkt.tcp.flags_syn==False) or (pkt.tcp.flags_syn==0) else 1)
                pf.features_list.append(0 if (pkt.tcp.flags_urg==False) or (pkt.tcp.flags_urg==0) else 1)
...
def process_pcap(pcap_file,dataset_type,bin_labels,mc_labels, max_flow_len,labelled_flows,max_flows=0, traffic_type='all',time_window=TIME_WINDOW):
    ...
    apply_labels(temp_dict, labelled_flows, bin_labels)
...
def process_live_traffic(cap, dataset_type, bin_labels, max_flow_len, traffic_type='all',time_window=TIME_WINDOW):
    start_time = time.time()
    temp_dict = OrderedDict()
    labelled_flows = []
...
def apply_labels(flows, labelled_flows, bin_labels):
    for five_tuple, flow in flows.items():
        if bin_labels is not None:
            ...
            if bin_label == 0:
                flow['label'] = 0 #mc_labels['benign']
                flow['label_string'] = 'benign'
            else:
                flow['label'] = 1 #mc_labels[traffic_label]
                flow['label_string'] = 'ddos' #traffic_label
...
def balance_dataset(flows,mc_labels,total_fragments=float('inf')):
    new_flow_list = []
    ...
    for flow in flows:
        if new_fragment_counters[flow[1]['label_string']] < min_fragments:
...
def main(argv):
    ...
    if args.preprocess_folder is not None or args.preprocess_file is not None:
        ...
        preprocessed_flows, fragment_counters = balance_dataset(preprocessed_flows,labels)
